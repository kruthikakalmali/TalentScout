    ...
    >
      ...
      >
        ...
        <div style={{ flex: '1 1 400px', maxWidth: '500px' }}>
          <h2 style={{ fontSize: '1.8rem', marginBottom: '20px' }}>📄 Job Descriptions</h2>
          {jobDescriptions.map((jd) => (
            ...
            >
              <h3 style={{ fontSize: '1.3rem', marginBottom: '10px' }}>{jd.title}</h3>
        ...
        <div style={{ flex: '1 1 400px', maxWidth: '500px' }}>
          ...
          {selectedJob ? (
            <>
              ...
              {matchCandidates(selectedJob.skills, applications).map((candidate) => (
                <div
                  key={candidate.id}
                  style={{
                    background: 'rgba(255, 255, 255, 0.1)',
                ...
                >
                  ...
                  <p style={{ opacity: 0.9 }}>💼 {candidate.skills.join(', ')}</p>
                  <p style={{ marginTop: '8px' }}>🔢 Match Score: {candidate.matchCount}</p>
                </div>
              ))}
              {matchCandidates(selectedJob.skills, applications).length === 0 && (
                <p style={{ opacity: 0.8 }}>No strong matches found.</p>
              )}
            </>
          ) : (
            <p style={{ opacity: 0.8 }}>Select a job description to view matches.</p>
          )}
        </div>
      </div>
    </div>
